...
suite('icon: helper', () => {
  test('returns a <span> with data-icon + class', () => {
    ...
    assertEqual(el.getAttribute('data-icon'), 'action.close', 'data-icon');
    assertEqual(el.classList.contains('icon'), true, 'icon class');
  });
  test('is decorative (aria-hidden)', () => {
    assertEqual(icon('action.close').getAttribute('aria-hidden'), 'true', 'aria-hidden');
  });
  ...
  test('extra class merged', () => {
    assertEqual(icon('action.close', { cls: 'big' }).classList.contains('big'), true, 'extra cls');
  });
});

suite('icon: raw variant', () => {
  test('raw:true adds .icon--raw class', () => {
    const el = icon('chart.pie', { raw: true });
    assertEqual(el.classList.contains('icon'), true, 'base class');
    assertEqual(el.classList.contains('icon--raw'), true, 'raw class');
    assertEqual(el.getAttribute('data-icon'), 'chart.pie', 'data-icon');
  });
  test('raw:true merges with cls', () => {
    const el = icon('chart.pie', { raw: true, cls: 'thumb' });
  ...
  test('without raw, no icon--raw class', () => {
    assertEqual(icon('action.close').classList.contains('icon--raw'), false, 'no raw by default');
  });
});
