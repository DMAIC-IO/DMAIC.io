...
suite('createScenarioModel', () => {
  ...
  test('apply() populates scenarioOptions for the preselected cycle, resets scenarioId', () => {
    const ScenarioModel = createScenarioModel(fakeOptionsFor);
    const m = new ScenarioModel().apply({ cycles, preselected: 'dmaic' });
  ...
  test('hasScenarios is true only when a real scenario exists beyond "empty"', () => {
    const ScenarioModel = createScenarioModel(fakeOptionsFor);
    const withScenarios = new ScenarioModel().apply({ cycles, preselected: 'dmaic' });
    assertTrue(withScenarios.hasScenarios);
    const withoutScenarios = new ScenarioModel().apply({ cycles, preselected: 'dmadv' });
